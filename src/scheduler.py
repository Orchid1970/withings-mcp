...
async def refresh_token_job():
    ...
    """
    try:
        from src.routes.admin import TokenRefreshService
        ...
        logger.error(f"[Scheduler] Could not import TokenRefreshService: {e}")
        ...
        try:
            ...
            if result.get("success"):
                logger.info(f"[Scheduler] Token refresh successful (alt path). Expires at: {result.get('expires_at')}")
            else:
                logger.error(f"[Scheduler] Token refresh failed: {result.get('error')}")
        except Exception as e2:
            logger.error(f"[Scheduler] All import paths failed: {e2}")
